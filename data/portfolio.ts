...
export const projects: Project[] = [
  ...
  {
    title: "team-ONE",
    description: "AI 팀원 추천·실시간 채팅 기반 팀 매칭 플랫폼 백엔드",
    longDescription:
      "공모전·사이드 프로젝트 팀원 모집 플랫폼의 사용자 백엔드 서버. FastAPI(Python) AI 서버와 연동해 프로젝트 요구사항 기반 팀원 추천, express-ws 기반 실시간 채팅(메시지·프로젝트 초대·이력서 카드 공유), Spring Boot 어드민 서버 자식 프로세스 통합 관리 기능을 구현했습니다.",
    type: "팀 프로젝트",
    role: "팀장 · 사용자 백엔드 서버 전담 개발 — JWT 기반 HTTP/WebSocket 통합 인증 미들웨어 설계, DB 스키마 모델링, WebSocket 채팅 시스템 구현, Spring Boot 자식 프로세스 통합",
    highlights: [
      "HTTP·WebSocket 공통 JWT 인증 미들웨어 — 헤더·쿼리 파라미터 양방향 토큰 처리",
      "FastAPI 연동 AI 팀원 추천 — 프로젝트 우선순위 기반 매칭 스코어 계산 후 DB 저장",
      "express-ws 실시간 채팅 — 메시지 브로드캐스트·프로젝트 초대·이력서 카드 공유",
      "Spring Boot를 자식 프로세스로 구동, /admin/* 요청 프록시로 단일 포트 서버 통합",
      "sanitize-html XSS 방지 레이어를 REST·WebSocket 양쪽에 일관 적용",
    ],
    techStack: ["Node.js", "Express 5", "JWT", "bcrypt", "MySQL", "WebSocket", "FastAPI 연동", "Winston"],
